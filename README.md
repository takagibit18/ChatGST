...
将后续真实 Markdown 放入 `knowledge/raw/`，补充 `knowledge/metadata/overrides.json` 后重新执行 `knowledge:validate` 与 `rag:build`。切片规则见 [chunking-strategy.md](docs/chunking-strategy.md)，输入审计见 [knowledge-audit.md](docs/knowledge-audit.md)。`knowledge/index/rag.db` 是生成物，不提交 Git。
