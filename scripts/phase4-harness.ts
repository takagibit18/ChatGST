...
async function finalReport() {
  ...
  const valueAt = (root: unknown, path: string): unknown => path.split(".").reduce<unknown>((value, key) => value && typeof value === "object"
    ? (value as Record<string, unknown>)[key] : undefined, root);
  const numeric = (root: unknown, path: string) => Number(valueAt(root, path) ?? 0);
  const fixed = (root: unknown, path: string) => numeric(root, path).toFixed(6);
  const frozenRows = valueAt(frozenResults, "results") as Array<Record<string, unknown>>;
  const frozenCandidate = frozenRows.find((item) => item.role === "candidate-a") ?? frozenRows[0] ?? {};
  const realRows = valueAt(realResults, "results") as Array<Record<string, unknown>>;
  const realCandidate = realRows[0] ?? {};
  const realResources = (valueAt(realCandidate, "resources") as Array<Record<string, unknown>> | undefined) ?? [];
  const realTokens = realResources.map((item) => Number(item.total_tokens ?? 0));
  const realMeanTokens = realTokens.reduce((sum, item) => sum + item, 0) / Math.max(1, realTokens.length);
  const realPerformance = (valueAt(realCandidate, "performance_ms") as Array<Record<string, unknown>> | undefined) ?? [];
  const realP95 = realPerformance.map((item) => numeric(item, "total.p95"));
  const realMeanP95 = realP95.reduce((sum, item) => sum + item, 0) / Math.max(1, realP95.length);
  const inventory = valueAt(frozenManifest, "case_inventory") as Record<string, unknown>;
  const isolation = valueAt(frozenManifest, "isolation") as Record<string, unknown>;
  const md = `# Phase 4 controlled experiments and validation closure\n\n## Gate\n\n- Phase 4 gate: **${phase4Gate}**\n- Production candidate gate: **${phase4Gate}**\n- 阻断：K0 冻结原文缺失；Frozen Test 失败；真实 DeepSeek 失败。\n- 生产部署：未授权、未执行。\n\n## 基线身份\n\n- Tag: \`${BASE_TAG}\`\n- Annotated Tag object: \`${TAG_OBJECT}\`\n- Peeled commit: \`${BASE_COMMIT}\`\n- Dataset manifest SHA-256: \`${sha256(datasetManifest)}\`\n- K4 hash: \`${K4_HASH}\`\n- 基线：Dev Document Recall@5 1.000000、Chunk Recall@5 0.934783、MRR@10 0.822464、nDCG@10 0.728830；Regression 13/13；Conversation/Safety 1.000000。\n\n## K0–K4 与 R1–R6\n\n- K0：冻结原始六文档从未提交，保留 \`blocked_missing_frozen_source\`，未伪造。\n- K1：Document Recall@5 0.804348，行为准确率 0.933333；朴素扩库引入明显召回噪声。\n- K2：Document Recall@5 0.891304，行为准确率 0.833333；仅元数据/地区标准化不足以恢复排序质量。\n- K3/K4：Document Recall@5 1.000000，MRR@10 0.822464，行为准确率 1.000000；K4 额外保留版本/权威性语义，作为后续唯一知识基线。\n- R1 将 MRR@10 提升到 0.862319，但行为准确率降至 0.966667；R2/R3 出现更大质量回退；R4–R6 未超过冻结基线门槛。\n- failure attribution 同时覆盖 retrieval/ranking/evidence sufficiency，现有证据不足以证明必须引入 dense 或 reranker。\n\n## Agent ablation\n\n12 个组件均运行 3 次且指纹一致，但没有一个通过硬门槛：A1 Regression 11/13，A2 12/13，A5 5/13；A8 关闭引用绑定后 citation precision/completeness 均为 0；A9 Conversation completion 0.900000；A10 关闭 stale guard 后 completion 0.500000 且 stale leakage 0.727273。地区层级、版本、安全预检和 stale guard 的高风险关闭配置禁止入选。\n\n## 候选\n\n- ${candidateLine(candidateA)}\n- Candidate A config hash: \`${candidateA.config_hash}\`\n- ${candidateLine(candidateB)}\n- Candidate selection commit: \`${String(valueAt(frozenManifest, "generation_commit"))}\`\n\n## Frozen Test\n\n- Annotated Tag object / peeled commit: \`${String(frozenTag.object ?? "unavailable")}\` / \`${String(frozenTag.peeled_commit ?? "unavailable")}\`\n- review status: \`machine_validated_unreviewed\`；Test SHA-256: \`${String(freeze.test_sha256)}\`\n- inventory：${String(inventory.total)} total = ${String(inventory.retrieval)} retrieval（${String(inventory.answerable)} answer / ${String(inventory.no_answer)} no-answer）+ ${String(inventory.conversations)} conversation + ${String(inventory.safety)} safety。\n- 隔离：prompt overlap ${String(isolation.exact_prompt_overlap_count)}，复用 Gold chunk ${String(isolation.reused_gold_chunk_count)}，source-first failure ${String(isolation.source_first_failures)}。\n- 状态 \`${String(frozenResults.status)}\`：behavior ${fixed(frozenCandidate, "metrics.retrieval.metrics.behavior_accuracy")}，answer recall ${fixed(frozenCandidate, "metrics.retrieval.metrics.answer_recall")}，no-answer recall ${fixed(frozenCandidate, "metrics.retrieval.metrics.no_answer_recall")}，Document/Chunk Recall@5 ${fixed(frozenCandidate, "metrics.retrieval.metrics.document_recall_at_5")} / ${fixed(frozenCandidate, "metrics.retrieval.metrics.chunk_recall_at_5")}。\n- fact coverage ${fixed(frozenCandidate, "metrics.retrieval.metrics.required_fact_coverage")}，citation precision/completeness ${fixed(frozenCandidate, "metrics.retrieval.metrics.citation_precision")} / ${fixed(frozenCandidate, "metrics.retrieval.metrics.citation_completeness")}；Conversation completion ${fixed(frozenCandidate, "metrics.conversations.scenario_completion_rate")}。\n- region/temporal/stale leakage 均为 0，Safety 1.000000；Test 结果生成后未继续调参。\n\n## 真实 DeepSeek\n\n- Provider/model: \`deepseek\` / \`${String(realResults.model_id ?? "unavailable")}\`；temperature 0；重复 3 次；状态 \`${String(realResults.real_model_evaluation)}\`。\n- 三次一致率 ${fixed(realCandidate, "consistency_rate")}；Frozen behavior/answer recall/no-answer recall ${fixed(realCandidate, "metrics.retrieval.metrics.behavior_accuracy")} / ${fixed(realCandidate, "metrics.retrieval.metrics.answer_recall")} / ${fixed(realCandidate, "metrics.retrieval.metrics.no_answer_recall")}。\n- Regression ${String(valueAt(realCandidate, "metrics.regression.behavior_correct"))}/${String(valueAt(realCandidate, "metrics.regression.cases"))}，Regression no-answer recall ${fixed(realCandidate, "metrics.regression.metrics.no_answer_recall")}；Conversation ${fixed(realCandidate, "metrics.conversations.scenario_completion_rate")}，stale 0，Safety 1.000000。\n- 结构化输出成功率每次 0.714286（2/7 失败），回答状态合法率和 citation-evidence binding 均为 1.000000。\n- token 总量三次为 ${realTokens.join(" / ")}（均值 ${realMeanTokens.toFixed(2)}）；total p95 均值 ${realMeanP95.toFixed(2)} ms；成本为 null，因为 provider 未配置可审计单价；重试/超时聚合计数 provider 未暴露。\n- 密钥未写入代码、日志或报告。\n\n## 诚实声明\n\nPhase 3.3 Gold 未修改；Phase 3 scorer 未修改；新增独立 Phase 4 scorer；未降低门槛；Frozen Test 未人审；未使用 Test 数据调参；未根据 Test 增加规则；真实 DeepSeek 已运行且失败；K0 未完成；未合并 PR 或部署生产。\n`;
