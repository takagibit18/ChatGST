...
const report={schema_version:1,run_id:raw.run_id,evaluation_status:"provisional",release_gate:"blocked_pending_human_review",quality_claim_allowed:false,circular_labeling:false,input_fingerprint:raw.input_fingerprint,prediction_fingerprint:raw.prediction_fingerprint,full_run_determinism:fullRunDeterminism,config:{...raw.config,required_fact_match:"normalized_bigram_recall>=0.45"},retrieval:{dev:scoreSet(dev),regression:scoreSet(regression)},conversations:{scenarios:conversations.length,turn_accuracy:divide(turnCorrect,turnTotal),scenario_completion_rate:divide(scenariosCorrect,conversations.length),stale_context_leakage_rate:divide(staleLeaks,switchTurns),confidence_95:wilson(scenariosCorrect,conversations.length),case_results:conversationResults},safety:{cases:safety.length,pass_rate:divide(safetyPassed,safety.length),false_refusal_rate:divide(falseRefusals,safety.length),confidence_95:wilson(safetyPassed,safety.length),case_results:safetyResults},performance_ms:{retrieval:{p50:percentile(timings,.5),p95:percentile(timings,.95),p99:percentile(timings,.99)},total:{p50:percentile(totals,.5),p95:percentile(totals,.95),p99:percentile(totals,.99)},warmups:2,measured:5},diagnostic_failures:[...scoreSet(dev).case_results.filter(item=>item.expected!==item.predicted||item.document_hit===false).map(item=>item.case_id),...conversationResults.filter(item=>!item.passed).map(item=>item.scenario_id),...safetyResults.filter(item=>!item.passed).map(item=>item.case_id)],review_notice:"All Gold remains pending_review; metrics are diagnostic only."};
await mkdir(resolve(root,"reports"),{recursive:true});await writeFile(resolve(root,"reports/phase3-v21-provisional.json"),`${JSON.stringify(report,null,2)}\n`,"utf8");console.log(JSON.stringify({scored:true,evaluation_status:report.evaluation_status,dev:report.retrieval.dev.metrics,conversations:report.conversations,safety:report.safety,diagnostic_failures:report.diagnostic_failures.length},null,2));
